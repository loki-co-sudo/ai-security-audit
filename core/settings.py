...
# 拡張ポートスキャン用の広めの well-known/サービスポート一覧（TCP）。
# COMMON より多くのサービスを拾うが、全ポート(1-65535)より十分高速。
EXTENDED_PORTS = sorted(set(COMMON_PORTS + [
    1, 7, 9, 13, 19, 26, 37, 79, 81, 88, 106, 111, 113, 119, 144, 179, 199,
    389, 427, 444, 465, 513, 514, 515, 543, 544, 548, 554, 587, 631, 646,
    873, 990, 993, 995, 1025, 1026, 1027, 1110, 1433, 1720, 1723, 1755,
    1900, 2000, 2001, 2049, 2121, 2717, 3128, 3268, 3986, 4444, 4899,
    5009, 5051, 5060, 5101, 5190, 5357, 5601, 5631, 5666, 5800, 5900, 5901,
    5984, 6000, 6001, 6443, 6646, 6667, 7001, 7070, 7077, 8008, 8009, 8081,
    8161, 8200, 8500, 8834, 9000, 9090, 9100, 9300, 9418, 9999, 10000, 11211,
    15672, 25565, 27018, 32768, 49152, 49153, 49154, 49155, 49156, 49157,
]))

# UDP スキャン用の代表的ポート（全UDPは非現実的なため厳選）。
# UDPは応答なし＝open|filtered の曖昧さがあるため検出は参考情報扱い。
COMMON_UDP_PORTS = [
    53, 67, 68, 69, 123, 137, 138, 161, 162, 500, 514, 520, 1900, 4500, 5353,
]

# ポートスコープ選択肢（ATTACK MODE のUIで選択。既定 common = 現行と同一挙動）。
PORT_SCOPES = ("common", "extended", "full")

...
> **注記**: 以下の全項目はアイディア段階であり、未実装です。実装の着手順序はロードマップを参照してください。

### ⚡ 速度改善（7件）

| # | アイディア | 優先度 | 難易度 | 期待効果 | 主な影響ファイル |
|---|-----------|--------|--------|----------|-----------------|
| 1 | CVE照合の並列化（`ThreadPoolExecutor`で全CWEをバッチ送出） | 🟢 P0 | ★☆☆ | CVE照合時間を約1/4に短縮 | `audit_agent.py`, `cve_client.py` |
| 2 | CVEキャッシュの永続化（`reports/cve_cache.json`、TTL=1日） | 🟢 P0 | ★☆☆ | 再スキャン時のCVE照合を0秒に | `cve_client.py`, `settings.py` |
...
| # | アイディア | 優先度 | 難易度 | 期待効果 | 主な影響ファイル |
|---|-----------|--------|--------|----------|-----------------|
| 1 | AST静的解析プリプロセッサの実質化（`ast.parse`で危険関数・構造を事前抽出→プロンプト注入） | 🟢 P0 | ★☆☆ | 検出漏れ（FN）削減・誤検知（FP）抑制 | `audit_agent.py` |
| 2 | 入力トランケーション戦略の改善（末尾優先/均等分割/importスキップ/制限値拡張） | 🟢 P0 | ★☆☆ | ファイル後半の脆弱性を検出可能に | `orchestrator.py`, `audit_agent.py` |
...
| 6 | 防御モニタリングのパターン集約フィルタ（同一IP・同一シグネチャをN分間隔に集約） | 🟢 P0 | ★☆☆ | LLM呼び出し回数を1/10〜1/100に削減 | `tools/monitor.py`, `monitor_agent.py` |
| 7 | 防御モニタリングのタイムウィンドウコンテキスト（直近30分のイベント履歴を添付） | 🟡 P1 | ★☆☆ | 多段階攻撃の検出率向上 | `monitor_agent.py` |
| 8 | Deep AnalysisのFP検証強化（`balanced` effortでも検証パスを実行） | 🟡 P1 | ★☆☆ | `balanced` effort時のFP率改善 | `audit_agent.py` |
| 9 | 偵察モード：既知サービスバナーのローカル照合（`known_vulns.yaml`でルックアップ） | 🟢 P0 | ★☆☆ | 既知サービスのLLMコストゼロ・ハルシネーション防止 | `tools/scanner.py`, `recon_agent.py` |

### 🛡️ 堅牢性・品質改善（3件）

| # | アイディア | 優先度 | 難易度 | 期待効果 | 主な影響ファイル |
|---|-----------|--------|--------|----------|-----------------|
| 1 | 設定バリデーション強化（`port_range`形式チェック・`timeout>0`・未知キー警告） | 🟢 P0 | ★☆☆ | 設定ミスによる実行時クラッシュを防止 | `core/settings.py` |

...
class App(ctk.CTk):
    ...
    def _set_icon(self) -> None:
        """ウィンドウアイコンを assets/ から読み込む。"""
        _assets = os.path.join(os.path.dirname(__file__), "..", "assets")
        ico = os.path.normpath(os.path.join(_assets, "icon.ico"))
        png = os.path.normpath(os.path.join(_assets, "icon.png"))
        try:
            ...
            if os.path.exists(png):
                ...
                _pil = Image.open(png).resize((64, 64))
                self._icon_img = ImageTk.PhotoImage(_pil)
